...
namespace SDL
{
	...
	textureData Engine::createTextSprite(const char* content, SDL_Color color)
	{
		...
		if (surface)
		{
			...
			if (texture)
			{
				...
				return {texture, (uint16_t) surface->w, (uint16_t) surface->h, spriteIndex++};
			}
			else
			{
				SDL_FreeSurface(surfaces[spriteIndex + 1]);
				surfaces.pop_back();
	...
	textureData Engine::createTextSpriteWrapped(const char* content, SDL_Color color)
	{
	...
	textureData Engine::createTextSprite(const char* content, SDL_Color fgColor, SDL_Color bgColor)
	{
	...
	SDL_Texture* Engine::createSprite(const char* file)
	{
...
/// <param name="shapeSize">the size of the shape (assumes its a square)</param>
	...
	void Engine::clearSpriteIndex(int index)
	{
		...
		for (int i = 0; i < allTextData.size(); ++i)
		{
			if (i >= index)
			{
				allTextData[i].index--;
